...
public abstract class DockerConventionJvmApplicationPlugin<EXT extends DockerConventionJvmApplicationExtension> implements Plugin<Project> {
    ...
    private TaskProvider<Dockerfile> registerDockerfileTask(final Project project, final EXT extension) {
        return project.getTasks().register(DOCKERFILE_TASK_NAME, Dockerfile.class, new Action<Dockerfile>() {
            @Override
            public void execute(final Dockerfile dockerfile) {
                ...
                dockerfile.from(project.provider(new Callable<Dockerfile.From>() {
                    ...
                    public Dockerfile.From call() throws Exception {
                        return new Dockerfile.From(extension.getBaseImage().get());
                    }
                ...
                dockerfile.copyFile(project.provider(new Callable<Dockerfile.CopyFile>() {
                    @Override
                ...
                dockerfile.entryPoint(project.provider(new Callable<List<String>>() {
                    @Override
    ...
    private static TaskProvider<Sync> registerSyncBuildContextTask(final Project project, final TaskProvider<Dockerfile> createDockerfileTask) {
        return project.getTasks().register(SYNC_BUILD_CONTEXT_TASK_NAME, Sync.class, new Action<Sync>() {
            @Override
    ...
    private TaskProvider<DockerBuildImage> registerBuildImageTask(final Project project, final TaskProvider<Dockerfile> createDockerfileTask, final EXT extension) {
        return project.getTasks().register(BUILD_IMAGE_TASK_NAME, DockerBuildImage.class, new Action<DockerBuildImage>() {
            @Override
    ...
    private static TaskProvider<DockerPushImage> registerPushImageTask(Project project, final TaskProvider<DockerBuildImage> dockerBuildImageTask) {
        return project.getTasks().register(PUSH_IMAGE_TASK_NAME, DockerPushImage.class, new Action<DockerPushImage>() {
            @Override
