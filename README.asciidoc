...
version 0.10.5 is used which assumes Docker's client API v1.16.
...
buildscript {
    ...
    dependencies {
        classpath 'com.bmuschko:gradle-docker-plugin:0.8.2'
    }
}
----
...
with https"] to set up your certificate. The following example demonstrates how to configure the plugin to use those certifactes.
