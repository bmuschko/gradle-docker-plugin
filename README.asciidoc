...
Docker does not need to be installed on the local or another remote machine. This project provides a Vagrant image with
the proper setup to bootstrap a Docker installation. The Vagrantfile can be found under the directory `vagrant`. To use
the Vagrant box simply start it manually. Alternatively, you can configure the project to bootstrap the Vagrant box
